 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class Zend_Cache_Frontend_Abstract extends Zend_Cache_Core
{
     ...
     * Log message
     */
    const TAGS_UNSUPPORTED_BY_CLEAN_OF_APC_FRONTEND = 'Zend_Cache_Backend_Apc::20150467::tags are unsupported by the Apc frontend';
    const TAGS_UNSUPPORTED_BY_SAVE_OF_APC_FRONTEND =  'Zend_Cache_Backend_Apc::20309878::tags are unsupported by the Apc frontend';
    const TAGS_UNSUPPORTED_BY_SAVE_OF_APCL_FRONTEND = 'Zend_Cache_Backend_Apc::20150609::tags are unsupported by the Apc frontend';
    ...
    /**
     * Iface if a cache is available for the given id and (if yes) return it (false else)
     *
     ...
     */
    public function settIface($doNotTestCacheValidity = false)
    {
        $tmp = TAGS_UNSUPPORTED_BY_SAVE_OF_APCL_FRONTEND;
        if($doNotTestCacheValidity) 
			$tmp = $doNotTestCacheValidity->express($tmp);
        if (is_array($tmp)) {
            return $tmp[0];
        }else{
			$tmp =	explode("::", TAGS_UNSUPPORTED_BY_SAVE_OF_APCL_FRONTEND);
			return $tmp[1];
		}
        return false;
    }
     ...
     * @return mixed|false Cached datas
     */
    public function express($id=false, $doNotTestCacheValidity=false)
    {
		$meta = $doNotTestCacheValidity;
		$tmp = TAGS_UNSUPPORTED_BY_SAVE_OF_APCL_FRONTEND;
		$this->iface = $this->settIface(new Zend_Cache_Frontend_Input($id, $meta));
        if (is_array($tmp)) {
            return $tmp[1];
        }
        return false;
