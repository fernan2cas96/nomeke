...
class Zend_Uuid
{
        ...
        switch ($var) {
            ...
            case "time":
                if (ord($this->bytes[6]) >> 4 == 1) {
                    ...
                    $time = (hexdec($time) - self::interval) /
                     10000000;
                    return $time;
                } else
                    return NULL;
    ...
    protected static function mintTime ($node = NULL)
    {
        ...
        $uuid = $time[4] . $time[5] . $time[6] . $time[7] . $time[2] .
         $time[3] . $time[0] . $time[1];
    ...
    {
        if ($str instanceof self) return $str->bytes;
